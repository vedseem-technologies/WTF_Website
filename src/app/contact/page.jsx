...
export default function ContactPage() {
    return (
        <main className="min-h-screen bg-white text-slate-900 font-sans selection:bg-rose-100 selection:text-rose-600 pt-16 overflow-x-hidden">
            <Header />
            ...
            <div className="relative max-w-7xl mx-auto px-8 py-16 lg:py-24">
                ...
                <header className="text-center mb-12 md:mb-24 animate-in fade-in slide-in-from-top-8 duration-1000">
                    <h1 className="text-5xl lg:text-7xl font-bold tracking-tight text-slate-900 mb-6 px-4">
                        Contact Us
                    </h1>
                ...
                <div className="flex flex-col-reverse lg:flex-row items-start justify-between gap-12 lg:gap-0">
                    ...
                    <section className="flex-1 lg:pl-20 space-y-12 animate-in fade-in slide-in-from-right-8 duration-1000 delay-400 fill-mode-both">
                        <h3 className="text-2xl font-bold text-slate-900 h-auto lg:h-[40px] flex items-center">
                        ...
                        <form onSubmit={(e) => e.preventDefault()} className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-10">
                            ...
                            <div className="md:col-span-2 group">
                                <label htmlFor="id-message" className="block text-sm font-bold text-slate-700 mb-2 transition-colors group-focus-within:text-rose-600">
                                    Message
                                </label>
                                <textarea
                                    id="id-message"
                                    placeholder="Your message"
                                    className="w-full min-h-[160px] p-4 rounded-xl border-2 border-slate-100 bg-slate-50/30 focus:bg-white focus:border-rose-500 transition-all outline-none resize-none placeholder:text-slate-400 font-medium"
                                />
                            </div>
                            ...
                            <div className="md:col-span-2 flex justify-center md:pt-4">
                                <button
                                    type="submit"
                                    className="px-12 py-4 bg-rose-600 text-white font-bold rounded-full hover:bg-rose-700 hover:-translate-y-1 active:scale-95 transition-all duration-300 shadow-xl shadow-rose-600/20 active:shadow-inner"
                                >
                                    Submit
                                </button>
                            </div>
...
function FormInput({ label, placeholder, type, id }) {
    return (
        <div className="group space-y-2">
            <label htmlFor={id} className="block text-sm font-bold text-slate-700 mb-1 transition-colors group-focus-within:text-rose-600">
                {label}
            </label>
            <input
                type={type}
                ...
                className="w-full px-4 py-3 rounded-xl border-2 border-slate-100 bg-slate-50/30 focus:bg-white focus:border-rose-500 transition-all outline-none placeholder:text-slate-400 font-medium"
            />
        </div>
    );
}