...
const getQuantityRules = (item) => {
  const isKg =
    item.measurement?.toLowerCase() === "kg" ||
    (typeof item.price === "string" &&
      item.price.toLowerCase().includes("/kg"));
...
const CateringSummaryView = ({
  ...
  const [bookingDetails, setBookingDetails] = useState(() => {
    if (typeof window !== "undefined") {
      ...
      return stored
        ? JSON.parse(stored)
        : initialBookingDetails || { date: "", time: "", vegGuests: "10" };
    }
    ...
    const fetchMenuSelection = async () => {
      ...
      try {
        ...
        const menuResponse = await fetch(
          `${BACKEND_URL}/api/menu-selection/${entityType}/${entityId}`,
        );
        ...
        if (menuData) {
          ...
          const unselectedItems = [
            ...(menuData.unselectedStarters || []).map((i) => ({
              ...i,
              category: i.category || "Starter",
            })),
            ...(menuData.unselectedMainCourses || []).map((i) => ({
              ...i,
              category: i.category || "Main Course",
            })),
            ...(menuData.unselectedDesserts || []).map((i) => ({
              ...i,
              category: i.category || "Dessert",
            })),
            ...(menuData.unselectedBreadRice || []).map((i) => ({
              ...i,
              category: i.category || "Bread & Rice",
            })),
          ];
          ...
          if (unselectedItems.length > 0) {
            setAvailableMenuItems((prev) => {
              ...
              const newItems = unselectedItems.filter(
                (u) => !existingIds.has(u._id || u.id),
              );
    ...
    const fetchAllMenuItems = async () => {
      try {
        const menuItemsResponse = await fetch(
          `${BACKEND_URL}/api/menu-items?limit=1000`,
        );
  ...
  React.useEffect(() => {
    ...
    return () => {
      window.dispatchEvent(
        new CustomEvent("hideBottomNavbar", { detail: false }),
      );
    };
  }, []);
  ...
  const liveServices = [
    ...
    {
      name: "Live Bread / Starters (Chef + Tandoor)",
      image: "/bread-live.png",
    },
  ...
  const proceedWithPayment = async (id) => {
    ...
    try {
      ...
      const response = await fetch(
        `${BACKEND_URL}/api/orders/payment/initiate`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
                ...
                {categoryFullMenu
                  ...
                  .map((item, idx) => {
                    return (
                      <div
                        key={idx}
                        className="flex items-center gap-4 p-3 border border-gray-100 rounded-2xl hover:bg-gray-50 transition-all"
                      >
                        <div className="w-16 h-16 rounded-xl bg-gray-200 relative overflow-hidden shrink-0">
                          <Image
                            src={item.image || "/block-1.png"}
                            alt={item.name}
                            fill
            ...
            return (
              ...
              >
                <motion.div
                  animate={{
                    backgroundColor:
                      isCompleted || isActive ? "#ef4444" : "#ffffff",
          ...
          {!showPriceBreakup && !showDetails && !showPayment ? (
            ...
            >
              <div className="bg-red-50/50 rounded-2xl p-6 mb-8 border border-red-100/50">
                <h3 className="text-2xl font-bold text-gray-900 mb-4 opacity-70 uppercase tracking-wider">
                  Booking Details
                </h3>
                <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
                  <div>
                    ...
                    {packageItem && (
                      <>
                        <p className="text-xl text-gray-500 font-semibold mb-1">
                          Package
                        </p>
              ...
              <div className="space-y-4 pb-12">
                {categoriesMap.map((cat) => {
                  ...
                  return (
                    <div
                      key={cat.label}
                      className="rounded-3xl bg-white/80 backdrop-blur shadow-sm hover:shadow-md transition-all border border-gray-50"
                    >
                      <button
                        onClick={() =>
                            ...
                            {isLiveService ? (
                              <div className="flex gap-4 overflow-x-auto pb-4 scrollbar-hide snap-x pt-6">
                                {liveServices.map((service, idx) => (
                                  <div
                                    key={idx}
                                    className="flex-shrink-0 w-36 h-44 bg-gray-50 rounded-2xl overflow-hidden relative snap-center border border-gray-100"
                                  >
                              ...
                              <div className="pt-6">
                                ...
                                <div className="space-y-3 pb-6">
                                  {categoryItems.map((item, idx) => (
                                    <div
                                      key={idx}
                                      className="flex items-center gap-4 p-4 bg-gray-50 rounded-2xl border border-gray-100/50"
                                    >
            ...
            >
                ...
                <div className="px-6 py-5 md:space-y-3">
                  ...
                  <div className="border-t border-gray-100 my-4 pt-4">
                    {items.map((item, idx) => (
                      <div
                        key={idx}
                        className="flex justify-between text-xl text-gray-600 mb-2"
                      >
                        <span>
                          {item.name}{" "}
              ...
              <div
                className="mt-6 flex gap-6 text-md text-gray-500"
                style={{ lineHeight: "0.7" }}
              >
              ...
              {!user ? (
                <div className="space-y-6">
                  <div className="mb-8">
                    ...
                    <p className="text-2xl text-gray-500">
                      Please {isLoginMode ? "login" : "sign up"} to proceed with
                      your booking
                    </p>
                  </div>
                  <form onSubmit={handleAuthSubmit} className="space-y-4">
                    ...
                    {!isLoginMode && (
                      <div className="space-y-2">
                        <label className="text-xl text-gray-500 block">
                          Addresses
                        </label>
                          ...
                          <div key={idx} className="flex gap-2">
                            ...
                            {idx > 0 && (
                              <button
                                ...
                                onClick={() =>
                                  setAuthAddresses(
                                    authAddresses.filter((_, i) => i !== idx),
                                  )
                                }
                        ...
                        {authAddresses.length < 3 && (
                          <button
                            type="button"
                            onClick={() =>
                              setAuthAddresses([...authAddresses, ""])
                            }
                            className="text-red-600 font-bold text-xl flex items-center gap-1"
                          >
                            <svg
                              className="w-5 h-5"
                              fill="none"
                  ...
                  <p className="text-center text-xl text-gray-500 mt-4">
                    ...
                    <button
                      ...
                      className="ml-2 text-red-600 font-bold underline"
                    >
                ...
                <div className="space-y-8">
                  <div className="mb-8">
                    ...
                    <p className="text-2xl text-gray-500">
                      Confirm your details and select delivery address
                    </p>
                  </div>
